...
**NEXT: 72 `signal-windows`** (07-2 cells 1–21: Window · Split · Normalize; its hosts and the 79.5% against 33.3% split measurement are under § *Slot 75* in the catalogue's arc section, the row still numbered 75 there) — measure → mock from `_lab/signal-cnn-lstm-mock.html`'s shell → his picks → draft. It trains nothing; its Split page can score 73's trained net through `widgets/signal-cnn-lstm/engine.js`. Then 74 `sequence-encoding`. The sequence arc is then complete.
