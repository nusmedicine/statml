...
**NEXT: his call.** Two things are on the table: (1) the title of 72 `signal-windows`, to be standardised on the arc's pattern — `_lab/signal-windows-title-mock.html` shows five candidates as gallery cards beside the arc (A · "Signals: Raw to Windows" recommended), awaiting his pick; (2) the 08 language arc, for which the attention head now in `signal-cnn-lstm/engine.js` (measured in `_lab/positional-encoding-measure.mjs`) is the first piece — nothing planned yet.
