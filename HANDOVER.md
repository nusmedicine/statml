...
**NEXT is his call, between two.** **The mid-press page switch in 69 `driver-genes`, which is live**, and a sweep of every paged widget for it: § *Slot 70* below has the fix pattern, and a task chip offered on 2026-09-19 carries the brief (the probe, one commit and one interrupted state per affected widget; a core option, a flag on the page parameter, if many are affected). **Or slot 71 `somatic-interactions`**, held back, measured and cuttable in the catalogue's § *Slot 71*. The cancer mutation arc's 67, 69 and 70 are complete; the image arc (PHM5005 06) has 61, 64, 65 and 62 shipped and 63 `pretrained` on KIV; slot 52 `training-loop` was discarded on 2026-09-13; the GWAS and PRS arc and the high-throughput arc are complete.
...
- **LIVE BUG, found 2026-09-19 from slot 70:** on *One gene*, press "Add the mutations" and switch to *The cohort* within a few frames, and the cohort's first press ("Drop the genes with no cluster") runs unasked, 29 frames, its button then reading "Place each by its fraction in clusters". Core keeps the loop running through the page change. § *Slot 70* has the fix pattern and the probe; the fix is one commit with an interrupted state, and the push is his.
