...
**SIXTY-SIX WIDGETS IN THE MANIFEST — 65 on the gallery and `roc-auc` UNLISTED; no draft on `main`** (`wgcna` is still a draft on its own branch and worktree, another session's). The live site is <https://nusmedicine.github.io/statml/>, deployed from `main`: the latest widget ship is `1fe6062` (72 `signal-windows`, 2026-09-21, pushed with the eleven commits before it as `2eb2aa7`, deploy 35522576250 green, the live manifest read back with 66 widgets and the new blurb), and this handover's commit follows it. **SESSION CLOSED.**
...
**2026-09-21, the next session, ask to continue planning: 74 `embedding-space` PLANNED, with a position page.** His ask — *main emphasis embedding space, but consider separate pages on positional encoding (for PHM5005 DL for Language): learned, fixed (frequency), relative (RoPE)* — read against 08-1 cell 1 §2 and 08-3 cell 4 §2 and his three position figures (the notebooks are at `~/Downloads/PHM5005 AY2025-26 - Notebooks/Master/`). Measured twice (`_lab/embedding-space-align-measure.mjs`: the raw per-epoch PCA reflects and spins, own-basis Procrustes to the final frame holds it; `_lab/positional-encoding-measure.mjs` on `_lab/attention-lab.js`, a single self-attention head with backward written for it: no position sits at 48–49% on an order task, each encoding learns it, the same tokens shifted by 24 positions leave rotary's scores unchanged to the last digit while learned falls to chance and sinusoidal below it; the learned table gets NO neighbour order, and with real tokens prepended every encoding falls, so the page shifts positions and claims nothing about longer sequences). Mocked (`_lab/embedding-space-mock.html`, everything trained on the page), and **the mock overturned two lines of the proposal**: the unrewarded codon rows DO move (1.50 against 2.01) and just land nowhere, and the codon axes must come from the eighteen rewarded rows (100% in 2-D against 44%). **Picked in four rounds, every one the recommendation:** three pages Amino acids · Codons · Position, the third with Encoding none · learned · sinusoidal · rotary (data) and Shift 0 · 24 (display); his figure's two panels moving as it trains; N(0, 1) start; E = 4 · 8 · 16; title "Deep Learning - Embedding Space"; pages 1 and 2 drafted first. The catalogue's § *Slot 74* has every number and pick.

**NEXT: the draft of 74 `embedding-space`, pages 1 and 2** (Amino acids · Codons, on 73's engine, the frame held by own-basis Procrustes to the final epoch, E = 8 and 40 epochs cached by data parameters), reviewed, then the Position page on the attention head moved from `_lab/attention-lab.js` into `signal-cnn-lstm/engine.js`. The picks are in the catalogue's § *Slot 74*. It is the arc's last; the sequence arc is then complete.
