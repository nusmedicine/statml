...
**SIXTY-TWO WIDGETS IN THE MANIFEST — 61 on the gallery and `roc-auc` UNLISTED; no draft on `main`** (`wgcna` is still a draft on its own branch and worktree, another session's). The live site is <https://nusmedicine.github.io/statml/>, deployed from `main`: the latest widget ship is `d411b2c`, and this handover's commit follows it.

**SESSION CLOSED 2026-09-18.** Slot 69 `driver-genes` **SHIPPED AND PUSHED** as `d411b2c`, on his "tested ok, push it to gallery, write handover and close this session". The session opened on "read handover and let's continue with planning the next widget", built the draft, and then took six rounds in one day, all local until the push, which carried the thirteen commits since `9df7754`. What the rounds settled: Colour by kind on at open; page 2's names clear of every point, ring and other name; **page 1 declines Play**, after he asked whether it made sense for steps that are read — measured, each press puts 12 to 80 words on screen that were not there before, 3.6 to 24 seconds of reading at 200 wpm, against Play's 1.1 s pause, and sharing the row also cost the step button the width its two longest labels need; **a core fix so a two-column grid option wraps instead of truncating**, which gave `hardy-weinberg` back "Heterozygotes miscalled" and `polygenic-score` two of its six steps; **four tweens he picked from `_lab/driver-genes-tween-mock.html`**, the largest being page 2 rebuilt from its own table in five presses, with Benjamini-Hochberg moving every gene by its rank; a second core fix so an anim-keyed step label may nest a `{ param }` label; and the copy audit — nine corrections, his four register calls, and the method-first subtitle. At the ship: 22 states, verify 66 checks, `check` and `test` read alone, the fronted suite 846 of 846 identical, the deploy green, and the live manifest `shipped`.

**Two lessons from the ship, both now in the order below.** The states must be recorded AFTER the status flip: the draft banner adds about 44px, which pushed widget 69's two tallest states past the harness's 1,200px frame onto a scrolled page at a 669px canvas, and the suite then named exactly those two (and the driven state ending on one of them) as DIFFER on `px` with `tx` identical. And a core change whose reach is the RAIL cannot move a hash — the rail is not hashed — but the suite was still run for each, twice at 824 of 824 and once at 846 of 846.

The 2026-09-17 closes — slot 62's Classification task at `ea9426e` and slot 67 `tumor-heterogeneity` at `93f2d6d` — are recorded in their sections below and in the catalogue.

**NEXT: slot 70 `mutational-signatures`.** The catalogue's § *Slot 70* holds the plan for 01-4 end to end: how a mutation becomes one of 96 trinucleotide categories, a tumour as a mix of signatures, and naming a signature by its nearest COSMIC reference; it imports widget 41's NMF engine rather than repeating it. Measure first (the lesson's own data load in base R — *The lesson's own data* under slot 67 below — and `_lab/cancer-plan-export.R` writes the TSVs), then mock, then his picks. Slot 71 `somatic-interactions` is his call once 70 is built. The image arc (PHM5005 06) has 61, 64, 65 and 62 shipped and 63 `pretrained` on KIV; slot 52 `training-loop` was discarded on 2026-09-13; the GWAS and PRS arc, the high-throughput arc and the cancer mutation arc's 67 and 69 are complete.
