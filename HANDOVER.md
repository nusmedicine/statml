...
**NEXT: 74 `sequence-encoding`** (07-3 cells 1–25: Tokenize · Pad · Encode; its hosts are under § *Slot 72* in the catalogue's arc section, the row still numbered 72 there, and the encoding strip is already drawn in `_lab/seq-arc-mock.html` § 5 and `_lab/sequence-cnn-lstm-encoding-mock.html`) — measure → mock from `_lab/signal-windows-mock.html`'s shell → his picks → draft. It is the arc's last; the sequence arc is then complete.
