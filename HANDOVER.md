...
**NEXT: 74 `embedding-space`** (replacing `sequence-encoding`; 07-1 cell 3's *Vector space* panel, 07-3's amino-acid vocabulary) — mock from `_lab/signal-windows-mock.html`'s shell (two pages, Amino acids by role and Codons by synonymy, the points moving as the model trains from the equidistant one-hot start, coloured by their true role; E a data control, 8 the default) → his picks → draft. **Before the first mock is shown, measure the frame-to-frame alignment of the projection** (Procrustes to the previous epoch), or the picture rotates as it trains. It is the arc's last; the sequence arc is then complete.
