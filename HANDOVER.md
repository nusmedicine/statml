...
**IN REVIEW, NOT PUSHED (2026-09-17, a later session): slot 62 `augmentation` has a Classification option on `main`, committed locally.** Kenneth asked whether augmentation is done for classification; MONAI was run, a mock drawn (`_lab/augmentation-classification-mock.html`) and the option built on his picks (catalogue § Slot 62, *CLASSIFICATION OPTION* and *BUILT*). **Do not push `main` until he has tested it and given the word:** it changes a shipped widget, and every push publishes. At ship it owes Classification fingerprint states, the full suite fronted, the subtitle question settled, and `task=classification` added to the widget's public link words below.
