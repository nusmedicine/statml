...
/**
 * One self-attention head with a position encoding, for widget 74's
 * Position page (PHM5005 08-1 cell 1 §2, 08-3 cell 4 §2). Written for the
 * lab on 2026-09-21 (`_lab/positional-encoding-measure.mjs`, checked against
 * finite differences to 2e-8) and moved here when the page was drafted.
 * Token-major, unlike the layers above: a head reads every token against
 * every other, and `[T][D]` is the shape that reads.
 *
 *   Embedding(V, D) + position → q, k (dk) and v (D) → softmax(q·k / √dk)
 *   → z = Σ α v → residual x + z → mean over tokens → Linear(D, 2)
 *
 * `pe`: "none" · "learned" (`nn.Embedding(Lmax, D)`, N(0, 1) rows, a row
 * never indexed never moves) · "sinusoidal" (the lesson's fixed table) ·
 * "rope" (nothing added; q and k rotated pair by pair by pos · θ_m, so the
 * score q·k depends on the offset alone). `x.pos`, if present, gives each
 * token's position; else 0..L−1 — the Position page's shift is the same
 * tokens with `pos` moved along.
 */
...
export function AttentionHead(rng, { V = 21, D = 16, dk = 8, Lmax = 48, pe = "none" } = {}) {
  const tok = param(rng, V * D, 0); for (let i = 0; i < V * D; i++) tok.v[i] = rng.normal(0, 1); for (let e = 0; e < D; e++) tok.v[e] = 0;
  const pos = pe === "learned" ? param(rng, Lmax * D, 0) : null; if (pos) for (let i = 0; i < Lmax * D; i++) pos.v[i] = rng.normal(0, 1);
  const sinT = pe === "sinusoidal" ? sinusoid(Lmax, D) : null;
  const Wq = param(rng, dk * D, 1 / Math.sqrt(D)), Wk = param(rng, dk * D, 1 / Math.sqrt(D)), Wv = param(rng, D * D, 1 / Math.sqrt(D));
  const headL = Linear(rng, D, 2);
  ...
  const model = {
    ...
    /** the pre-softmax scores among the tokens, one row a query */
