...
/* --- the stage ------------------------------------------------------------ *
 * n samples, every pair as far apart as it can be — which for these counts are
 * the shapes anyone would draw: a triangle, a regular tetrahedron, a triangular
 * bipyramid, an octahedron.
 *
 ...
 * and a triangle is flat, while four points all the same distance apart cannot
 * be drawn on paper at all. Verified over 200 starting layouts: n = 3 lands on
 * stress 0.000 every time, n = 4 on 1.830 every time.                        */
function spread(n) {
  const eq = (k, m) => {
...
function layout(w) {
  ...
  return {
    ...
    flat: { x: x0 + side + table + GAP * 2, y: TOP, w: side, h: side },
    height: TOP + side + BOT,
  };
}
...
function sampleDot(ctx, colors, x, y, col, hollow = 0, r = R_DOT) {
  ctx.save();
  ctx.beginPath();
  ctx.arc(x, y, r, 0, Math.PI * 2);
  if (hollow < 1) {
    ctx.globalAlpha = 1 - hollow;
    ctx.fillStyle = col;
    ctx.fill();
  }
  ctx.lineWidth = 1.6;
  ctx.strokeStyle = hollow > 0 ? colors.ink3 : colors.surface;
  ctx.globalAlpha = hollow > 0 ? 0.35 + 0.65 * (1 - hollow) : 1;
  ctx.stroke();
  ctx.restore();
}
...
defineWidget({
  ...
  params: {
    ...
    points: {
      ...
      label: "Samples",
      options: [
        { value: "3", label: "3", detail: "3 distances, all the same" },
        { value: "4", label: "4", detail: "6 distances, all the same" },
        { value: "5", label: "5", detail: "10 distances, three different lengths" },
        { value: "6", label: "6", detail: "15 distances, two different lengths" },
      ],
      default: "4",
    },
    seed: {
      ...
      max: 200,
      default: 1,
      detail: "where the samples are dropped before the first step",
    },
  ...
  compute({ params, rng }) {
    ...
    return { n, pts, D, path, pairs: pairList(n), span: span * 1.1 };
  },
  ...
  draw({ ctx, colors, w, params, state, anim }) {
    ...
    /* ---- left: where the samples are ------------------------------------ */
    {
      ...
      /* The axes go out with the coordinates. Leaving them lit past the point
         the table becomes the input invites reading a sample off a gene, which
         is exactly what MDS no longer has access to. */
      if (away < 0.99) {
        ctx.save();
      ...
      /* Back to front, so near samples occlude far ones. Without it the cloud
         is a flat sticker and turning it tells you nothing. */
  ...
  summary({ params, state, anim }) {
    ...
    if (!params.measured) {
      return `${n} samples plotted against three genes, spread as far from each other as `
        + `they can get. ${view} No distances measured yet.`;
    }
