...
const S = {
  ...
  clsLabel: "Class",
  clsDetail: "which class the sequence shown is drawn from: G and C at a fraction of 0.7, or of 0.3",
  codeLabel: "Encoding",
  codeDetail: "how a token reaches the first layer: four fixed one-hot rows, or a lookup table of eight learned values a token",
  seedLabel: "Seed",
  ...
  stopDetail: "where the window sits once it has slid; a click on a letter or on the rows picks it",
  ...
  modelLabel: "Model",
  modelDetail: "which trained model the windows are removed from",
  ...
  speedOptions: [
    ...
    { value: "fast", label: "Fast", detail: "a press in about a second, the first press of the 1D-CNN page in two" },
  ],
  heatLabel: "Heatmap",
  ...
  stepTitles: {
    ...
    lstm2: "Predict from each prefix: the model's probability if the sequence ended at that base",
    ...
    combo3: "Predict from each prefix: the model's probability if the maps ended at that step",
    combo4: "Every stage of this page is drawn",
    occ: "Set the next window of tokens to PAD, predict again, and record the change",
  },
  ...
  legend: {
    ...
    combo: [
      { token: "value-high", label: "A positive output of the recurrence" },
      { token: "value-low", label: "A negative output" },
      { token: "highlight", label: "The map column being read, and the states the linear layer reads" },
      { token: "empirical", label: "The model's probability if the maps ended at that step", mark: "line" },
      { token: "theory", label: "Where the motif is" },
    ],
    occlusion: [
  ...
  capSpell: (spell, match, k, offset) => `column maxima ${spell} · ${match} of ${Math.min(k, M.MOTIF.length)} match the motif${offset ? ` at a shift of ${offset > 0 ? "+" : ""}${offset}` : ""}`,
  ...
  capChain: "The stack, one output's receptive field, and the global max",
  ...
  capLoss: (n, e, r, p) => `loss by epoch · ${n} new sequences an epoch · ${e} epochs · the best of ${r} initialisations, chosen at epoch ${p}`,
  capLossShort: (n, e, r) => `loss by epoch · ${n} new an epoch · ${e} epochs · the best of ${r} initialisations`,
  capLossLstm: (n, e) => `loss by epoch · ${n} new sequences an epoch · ${e} epochs · H = ${SPEC.H}`,
  ...
  capNoMotifLstm: "this recurrence does not find the motif at H = 8; it reads composition and stays at chance",
  capFx: (p, model) => `f(x) = ${p.toFixed(2)} · f is the trained ${model}`,
  capOccluded: (p, a) => `window at PAD: f = ${p.toFixed(2)} · change ${a.toFixed(2)}`,
  capAttr: (k, s) => `attribution by position · k = ${k}, stride ${s} · PAD positions excluded`,
  capHalfMax: (n) => `width at half maximum ${n} bases`,
  capMotif: "the motif",
  ...
  tileReach: "Receptive field after Conv2",
  tileReachNote: "bases one output of the second convolution reads",
  ...
  tileParityNote: "held-out accuracy on motif sequences with the motif at an even base, then at an odd one",
  tileWait: "—",
  tileP: (name) => `p(${name})`,
  ...
  tileMove: "Drift over the padding",
  ...
  tileWidth: "Width at half maximum",
...
defineWidget({
  ...
  legend: ({ params }) => {
    const L = S.legend[params.page] ?? S.legend.cnn;
    /* under the learned encoding there is no one-hot cell on the figure */
    return params.page === "cnn" && params.code === "learned" ? L.map((e, i) => (i === 0 ? { ...e, label: "Each feature map drawn light to dark" } : e)) : L;
  },
