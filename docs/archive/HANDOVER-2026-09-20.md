...
**NEXT: his "push it" for 73 `signal-cnn-lstm` — RECORDED 2026-09-20 (thirty states, shooter proof 7 / 7, all STABLE, probe 15 / 0, status shipped, check · test · build pass; NOT PUSHED, NOT COMMITTED), the copy audit done, the subtitle and blurb his picks (catalogue, under 73). His earlier looks: at `http://localhost:8010/widgets/signal-cnn-lstm/` after round 1 (2026-09-20: maps aligned to the trace, a multiplication band, LSTM and CNN + LSTM as two pages each TRAINED in the browser at H = 8 with a running prediction; then the trace added to both LSTM pages, Ectopic beats 0 · 1 · 2 · 3 with 0 the sinus-rhythm class, eight epochs for the raw LSTM, the block drawn as h→ / h← halves with the reduced vector as two groups and what each reduction read, and a Model control on the occlusion page (1D-CNN · LSTM · CNN + LSTM, the model named in the caption); the catalogue's ROUND 1 paragraphs under 73 have every comment and what each changed).** The draft was built 2026-09-20 to his eight picks (the catalogue's § *The sequence arc*, under 73, *DRAFTED*): three pages, the CNN trained in `compute()` and cached, the LSTM forward-only at init, occlusion walked window by window; `check` and `test` pass; four placeholder fingerprint states; NOT committed. His eight picks on the mock (`_lab/signal-cnn-lstm-mock.html`) are in the catalogue's § *The sequence arc* under 73: every recommendation but the slug — Sinus rhythm / Ectopic beat, the CNN trained in `compute()` at half width, the LSTM page at init, k = 8 · 16 · 32 · 64 with a baseline control, k = 3 · 7 · 15 · 31 in samples with ms, H = 8, copies shared. 73 is MEASURED (`_lab/signal-cnn-lstm-measure.mjs`, 4 s: the half-width net trains in 1.0–1.5 s, the LSTM cannot be trained in the browser, the ectopic-beat stage holds) and its engine and model modules exist (`widgets/signal-cnn-lstm/engine.js`, `model.js`; no `main.js` yet). The sequence arc (PHM5005 07-1 to 07-3) was PROPOSED, MEASURED AND PICKED the same day. His picks: group **by data type, two widgets each** — 72 `signal-windows` (07-2 cells 1–21: Window · Split · Normalize), 73 `signal-models` (07-2 cells 22–99: 1D-CNN · LSTM · Occlusion, CNN + LSTM as a strip on the LSTM page), 74 `sequence-encoding` (07-3 cells 1–25), 75 `sequence-models` (07-3 cells 26–178: the same three pages on tokens); build order 73 → 72 → 75 → 74; the Reduce control a display of what each reduction computes; the LSTM by name, no cell control. The catalogue's § *The sequence arc* has the covered-already table, the measured claims (now the pages' claims), the four groupings that were put to him (`_lab/seq-arc-mock.html` § 7) and the picks. The measure script is `_lab/dl-seq-measure.mjs` (106 s, fourteen engines gradient-checked, findings in its header); his fourteen figures are in `_lab/figs/dl-sequential-*.png`. **Slot 71 `somatic-interactions` CUT 2026-09-20 on his word**, and slot 52 `training-loop` was already discarded on 2026-09-13; the request named both. None of this is committed yet. 63 `pretrained` stays on KIV; the cancer mutation arc's 67, 69 and 70 are complete; the image arc (PHM5005 06) has 61, 64, 65 and 62 shipped; the GWAS and PRS arc and the high-throughput arc are complete.
...
## The sequence arc — planned 2026-09-20, awaiting his picks

**His ask:** *read handover and let's plan the next set of widgets to support PHM5005 Deep Learning for Sequences. Can scrap the other planned widgets training-loop and somatic-interactions.* The three notebooks (`07-1 Overview`, `07-2 Signals`, `07-3 Sequences`) were read in full from the Master copies; two facts were fetched rather than assumed (PhysioNet: the ECG fragments are 2 s at 360 Hz; the lesson's DNA CSV: 100,000 sequences, every one exactly 200 bp, so `MAX_LEN = 250` pads all and truncates none). The catalogue's § *The sequence arc* has the covered-already table, the five slots with hosts and measured claims, the engine argument, the order, and the six open calls. Three things the measurement changed before anything was drawn, so the mock does not carry them: the RNN-against-LSTM memory comparison does not separate at H = 16 (a trained tanh RNN recalled the first token as far as the LSTM), so 74 names the LSTM and offers no cell control; the lesson's pooling table (last for endpoints, mean for the trend, max for the event) reproduced only its max row, so 74's Reduce control shows what each reduction computes on the one task where it matters rather than a claim per arm; and the first split experiment's feature (raw correlation) under-read the leak because overlap in samples is not similarity at zero lag — the log power spectrum, which a model would learn, gives 79.5% against 33.3%.

**He answered the same day** (the catalogue section's *His answers* and *PICKED* paragraphs): by data type, two widgets each; `signal-models` first; CNN + LSTM a strip; Reduce a display; the LSTM by name. The concept cut's hosts and measurements are now the pages of the four widgets. The next step is 73 `signal-models`: the conv1d, RNN and LSTM engine lifted out of `_lab/dl-seq-measure.mjs` into `widgets/signal-models/engine.js` (75 `sequence-models` will import it), its own measurement, then its mock from `_lab/mutational-signatures-mock.html`'s shell, then the draft. The mock page is `_lab/seq-arc-mock.html`; every number on it is computed on the page from the same arithmetic as the script, except where a panel's caption says it is the script's figure.
